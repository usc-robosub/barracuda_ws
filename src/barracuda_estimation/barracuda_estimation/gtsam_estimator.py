...
except ImportError:  # pragma: no cover - runtime fallback for Jetson/Docker images without gtsam
    gtsam = None
    B = V = X = None
...
class GtsamEstimator:
    """
    Backend scaffold for the future Barracuda GTSAM estimator.

    This class intentionally keeps ROS2 concerns out of the core estimation flow.
    The current implementation already supports an early graph-update path when
    the Python `gtsam` package is available, while still falling back to a
    lightweight DVL-seeded estimate in environments without GTSAM.
    """
    ...
    def step(self, stamp_sec: float) -> Optional[StateEstimate]:
        ...
        Today this supports:
        - a fallback path without GTSAM
        - an early online-style GTSAM path with IMU preintegration, depth
          constraints, DVL velocity constraints, and batch optimization
        ...
        """
        if not self.is_ready() or self.latest_dvl is None:
            return None

        if self.gtsam_available:
            estimate = self._step_with_gtsam(stamp_sec)
        else:
            estimate = self._step_without_gtsam(stamp_sec)

        self.latest_state = estimate
        self.imu_buffer.clear()
        return estimate

    def _step_without_gtsam(self, stamp_sec: float) -> StateEstimate:
        estimate = StateEstimate(
            stamp_sec=stamp_sec,
            position_xyz=self.latest_dvl.position_xyz,
            orientation_xyzw=self.latest_dvl.orientation_xyzw,
            velocity_xyz=self.latest_dvl.velocity_xyz,
        )
        return estimate
    ...
    def _pose3_from_measurements(self):
        ...
        x, y, _ = self.latest_dvl.position_xyz
        qx, qy, qz, qw = self.latest_dvl.orientation_xyzw
        rot = gtsam.Rot3.Quaternion(qw, qx, qy, qz)
        z = -float(self.latest_depth.z_value)
        return gtsam.Pose3(rot, gtsam.Point3(float(x), float(y), z))
    ...
    def _depth_factor(self, key_index: int, pose) -> Any:
        ...
        return gtsam.GPSFactor(
            X(key_index),
            gtsam.Point3(float(pose.x()), float(pose.y()), float(pose.z())),
            depth_noise,
        )
